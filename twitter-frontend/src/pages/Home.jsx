...
const Home = () => {
  ...
  const fetchTweets = async () => {
    try {
      await axios.get("http://localhost:3003/api/tweet").then((res) => {
    ...
    isLoading ? <Loader/> : 
      <div className='h-screen w-1/2 bg-white m-1 relative overflow-x-hidden'>
        ...
        {tweets.map((val) => {
          ...
          return (
            <div onClick={()=>{navigate(`/tweet/${val._id}`)}} className='cursor-pointer hover:bg-blue-100 m-4 border rounded-sm p-3'>
              {val.retweetBy.length > 0 ? 
              <p className='px-14 text-gray-500 text-sm'><i className='fa fa-retweet text-gray-500'></i>{` Retweeted by ${val.retweetBy[val.retweetBy.length - 1].username}`}</p>
              : ''}
