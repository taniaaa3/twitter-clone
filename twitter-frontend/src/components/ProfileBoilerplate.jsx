...
const ProfileBoilerplate = (props) => {
  ...
  return (
    <div className='h-screen w-5/6 sm:w-1/2 bg-white m-1 relative overflow-x-hidden p-2 sm:p-6'>
      <header className='flex flex-row justify-between items-center'>
        <h1 className='text-lg font-semibold'>Profile</h1>
      </header>
      <div className='bg-blue-600 h-36 mt-2 '>
        <div className='w-max h-auto relative top-2/3 left-1 sm:left-5'>
      ...
      <div className=' flex flex-row items-center justify-end'>
        {props.admin ?
          <div className='w-1/2 mt-1 flex flex-col sm:flex-row justify-center items-center'>
                  ...
                  <div className=''>
                    <p className='font-semibold'>@{val.tweetedBy.username} - <span className='text-gray-500'>{new Date(val.createdAt).toDateString()}</span>
                      {val.tweetedBy._id == user._id ?
                        <span className='absolute right-1 sm:right-5 cursor-pointer'><i className='fa fa-trash' onClick={(e) => { e.stopPropagation(); deleteTweet(val) }}></i></span>
                        : ""
                      }</p>
                    <p>{val.content}</p>
                  </div>
